...
\begin{rtable}
  ...
  \begin{rsection}{Recent\\Research\\Highlights}
    \begin{rentry}{Demikernel}{}
      \textit{A new datapath OS and architecture for
...
\begin{rentry}{Capybara}{}
  \textit{A microsecond-scale load balancer that uses TCP
  migration to move established TCP connections between
  servers.} Capybara uses a programmable switch to implement
  transparent L4 load balancing and a user-space TCP stack to
  encapsulate and move application-level state. Using this co-designed
  architecture, Capybara achieves up to 149x lower tail latency and
  more than 2x higher throughput for scale-out services compared to
  state-of-the-art load balancing approaches.
    \end{rentry}
    
    \begin{rentry}{Cornflakes}{}
      \textit{A memory-safe, zero-copy serialization library for
        microsecond-scale applications.} Serialization requires data
      movement -- achieved today using copies -- that is at odds with
      kernel-bypass networking stacks that aim to eliminate copies
      from the I/O path.  Network devices are better suited for data
      movement than CPUs but existing scatter-gather APIs are poorly
      designed for data serialization.  Cornflakes is a hybrid
      serialization stack that uses NIC scatter-gather when it is
      better and otherwise falls back to software copies.
    \end{rentry}

    \begin{rentry}{Treehouse}{}
      \textit{A new software-centric approach to green datacenters.}
      While existing research aims to reduce energy usage of
      datacenter hardware, Treehouse argues that being energy-aware in
      the design of datacenter software is equally important. To that
      end, Treehouse aims to make energy and carbon visible to
      application developers on a fine-grained basis, modify system
      APIs to make informed trade offs between performance and carbon
      emissions, and raise the level of application programming for
      flexible use of more energy efficient means of compute and
      storage.
    \end{rentry}
  \end{rsection}    
\end{rtable}
...
\begin{rtable}
  ...
  \begin{rsection}{Conference\\Publications}
   I. Choi, N. Wadekar, G. Sun, R. Joshi, J. Fried, O. S. Navarro
   Leija, D. Ports, I. Zhang, J. Li. \textit{Capybara: Dynamic Load
     Balancing with Microsecond-scale TCP migration}. Proc of the ACM
   Special Interest Group on Data Communication (SIGCOMM 2026). August
   2026.  \\\vspace{-0.5em}
    
    K. Lim, M. Giordano, T. Stavrinos, \textbf{I. Zhang}, J. Nelson,
    B. Kasikci, T. Anderson. \textit{Beehive: A Flexible
    Network Stack for Direct-Attached Accelerators.} Proc. of the IEEE/ACM Symposium
    on Microarchitecture (MICRO). November 2024. \\\vspace{-0.5em}
...
\textbf{I. Zhang}, A. Szekeres, D. Van Aken, I. Ackerman,
  ...
  \begin{rsection}{Journal\\Publications}
    \textbf{I. Zhang}, N. K. Sharma, A. Szekeres, A. Krishnamurthy,
  ...
  \begin{rsection}{Workshop\\Publications}
    I. Choi, A. Bonde, J. Liu, J. Fried, \textbf{I. Zhang},
    J. Li. \textit{ML-native Dataplane Operating Systems.} Proc. of
    ACM SIGOPS Asia-Pacific Workshop on Systems (APSys). October
    2025. \\\vspace{-0.5em}

    K. Lim, M. Giordano, I. Zhang, B. Kasikci, T. Anderson. (2025,
    May). Apiary: An OS for the Modern FPGA. Proceedings of the
    Workshop on Hot Topics in Operating Systems (HotOS). May
    2025.\\\vspace{-0.5em}

    I. Choi, N. Wadekar, R. Joshi, J. Fried, D. R. K. Ports,
    \textbf{I. Zhang}, and J. Li. \textit{Capybara: Microsecond-Scale
      Live TCP Migration.}  Proc. of ACM SIGOPS Asia-Pacific Workshop on
...
\begin{rtable}
  ...
  %%   % \begin{rentry}{SOSP Distinguished Artifact Award}{2023}
  %%   %  \vspace{-0.8em}
  %%   % \end{rentry}
  ...
  \begin{rsection}{Service}
    ...
    \begin{rentry}{VEE, Program Committee}{2018}
       \vspace{-0.7em}
    \end{rentry}
    \begin{rentry}{ASPLOS, External Review Committee}{2017}
       \vspace{-0.7em}
    \end{rentry}
    \begin{rentry}{OSDI, External Review Committee}{2016}
       \vspace{-0.7em}
    \end{rentry}
...
%   \end{rsection}
    ...
    \textit{Saving and Restoring State Information for Virtualized
      Computer Systems.} 
    \textbf{I. Zhang}, K. C. Barr, G. Venkitachalam, I. Ahmad, A. Garthwaite, J. Pool.\\\vspace{-0.7em}
